# -*- coding: utf-8 -*-
"""
PyTorch 1D CNN for Arabidopsis TE prediction and cross-species comparison.
...
The Arabidopsis source names ``kozak`` and ``cds_length`` are renamed internally
...
Run:
    python pytorch_cnn1d_ara_cross_species.py --file "C:/path/data.xlsx"
"""
...
FILE = (
    ...
    r"xgboost_ara_filteredRNAgt10.xlsx"
)
SHEET_FEATURES = "filtered_tezheng"
SHEET_TARGET = "filtered_TE"
RESULT_DIR = "pytorch_cnn1d_ara_cross_species_results"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Arabidopsis PyTorch 1D CNN for cross-species comparison"
    )
...
def main():
    ...
    X, y, genes, aligned = load_and_align(
    ...
    validation_loader = make_loader(X_val, y_val, args.batch_size, shuffle=False)
    ...
    model_path = os.path.join(args.result_dir, "pytorch_cnn1d_ara_model.pt")
    torch.save(
        {
    ...
    config = {
        ...
        "source_to_canonical_mapping": FEATURE_MAPPING,
        "canonical_feature_order": CANONICAL_FEATURES,
        "aligned_sample_count": int(len(aligned)),
